...
Standalone startup accepts only the version-3 `__topic_metadata.json` manifest and recreates topic definitions/partition handlers before coordinator initialization. Corrupt, duplicate, unknown-version, incomplete, or semantically invalid metadata fails startup. A missing manifest with persisted topic logs, or a manifest that omits a persisted topic directory, also fails startup rather than silently losing or recreating data; restore a current manifest or archive the orphaned storage before retrying. An epoch greater than 1 requires a matching synced marker in the topic directory or recovery keeps the topic fenced and rebuilds its empty generation. Distributed recovery accepts only Raft snapshot version 9 with explicit topic definition fields and authoritative committed-HWM markers. Earlier persisted formats require a clean bootstrap; runtime never synthesizes their missing state.
