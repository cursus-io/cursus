...
Standalone startup first loads the versioned `__topic_metadata.json` manifest and recreates topic definitions/partition handlers before coordinator initialization. Corrupt, duplicate, unknown-version, or semantically invalid metadata fails startup. A missing manifest with persisted topic logs, or a manifest that omits a persisted topic directory, also fails startup rather than silently losing or recreating data; restore the compatible manifest or archive the orphaned storage before retrying. Epoch-1 state keeps manifest version 2 for rolling-upgrade compatibility. Manifest version 3 is selected after truncate; an epoch greater than 1 requires a matching synced marker in the topic directory or recovery keeps the topic fenced and rebuilds its empty generation. Distributed snapshots likewise remain version 7 at epoch 1 and advance to version 8 for lifecycle-fenced state. Version 7 receives lifecycle epoch 1 when absent, version 6 also receives revision 1 and infers replication factor from consistent replica metadata, while older snapshots reconstruct only the metadata those formats retained.
