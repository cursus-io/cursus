...
Exactly one selector is required. The broker durably stores the selector and its concrete topic-partition expansion. A group member receives each subscribed topic-partition at most once per generation.
...
For a multi-topic group, omit `topic`; the response uses `topic_assignments=<topic>:P<partition>,...`.
...
Success: `OK transactional_id=<id> appended=true topic=<topic> partition=<N> state=open`.
...
Success: `OK transactional_id=<id> mode=<legacy|exactly_once> state=<open|prepare_commit|prepare_abort|committed|aborted> messages=<N> participants=<N> offsets=<N>`.
