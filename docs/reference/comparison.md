...
| Storage | Append-only segments, sparse offset index, mmap reads, configurable rolling, time/size deletion, gated standalone/distributed keyed compaction, corruption recovery | Distributed compaction requires broker lifecycle protocol v2, full ISR, and matching authoritative HWM/lifecycle/policy; event-sourcing and internal metadata topics are excluded. A general timestamp index is not implemented. |
...
A more mature or specialized platform may be a better fit when the deployment requires a very large connector ecosystem, established multi-datacenter tooling, many assignment strategies, tiered storage, or independently audited long-duration failure evidence today. Distributed compaction is implemented, but it is newer than the standalone path and should be validated with the deployment's replica recovery and rolling-operation workload.
