...
func TestHelpResponseOrderContract(t *testing.T) {
	ch, _ := newTestHandler(t)
	const want = "OK commands=CREATE,DELETE,TRUNCATE,LIST,PUBLISH,CONSUME,STREAM,JOIN_GROUP,SYNC_GROUP,LEAVE_GROUP,HEARTBEAT,COMMIT_OFFSET,BATCH_COMMIT,FETCH_OFFSET,LIST_OFFSETS,INIT_PRODUCER_ID,BEGIN_TXN,TXN_PUBLISH,SEND_OFFSETS_TO_TXN,END_TXN,TXN_STATUS,REGISTER_GROUP,GROUP_STATUS,LIST_GROUPS,DESCRIBE,APPEND_STREAM,READ_STREAM,SAVE_SNAPSHOT,READ_SNAPSHOT,STREAM_VERSION,METADATA,FIND_COORDINATOR,LIST_CLUSTER,CLUSTER_STATUS,ELECT_LEADER,HELP,EXIT,ALTER_TOPIC_CONFIG,AGGREGATE_REPLAY_PROOF,AGGREGATE_EVENT_RANGE_READ"
