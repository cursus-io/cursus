...
func TestTransactionMarkerRequiresControlBatchMetadata(t *testing.T) {
	...
	msg.ControlBatchVersion = types.ControlBatchVersionKafkaV2
	msg.ControlBatchCoordinatorEpoch = tx.Epoch
	key, value, err := kafkaTransactionMarkerBytes(types.TransactionMarkerCommit, tx.Epoch)
	if err != nil {
		t.Fatalf("kafkaTransactionMarkerBytes failed: %v", err)
	}
