...
func (tm *TopicManager) AlterTopicMinInSyncReplicas(name string, value *int, replicationFactor int) error {
	...
	definition.Policy.MinInSyncReplicas = nil
	if value != nil {
		copyValue := *value
		definition.Policy.MinInSyncReplicas = &copyValue
	}
	policy, err := definition.Policy.Normalize()
	if err != nil {
		return err
	}
	return existing.applyDefinition(definition.Partitions, policy, tm.hp, tm.persistDefinitionLocked)
}
