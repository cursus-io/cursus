...
func (p *Publisher) sendBatch(part int, batch []types.Message) {
	...
	util.Debug("Sending batch %s: partition=%d, messages=%d, epoch=%d, seqRange=%d-%d", batchID, part, len(batch), p.producer.Epoch, batchStart, batchEnd)
	...
	data, err := util.EncodeBatchMessages(p.config.Topic, part, p.config.Acks, p.config.EnableIdempotence, batch)
	if err != nil {
