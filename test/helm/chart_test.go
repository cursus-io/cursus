...
func TestHelmStandaloneAndClusterContracts(t *testing.T) {
	...
	data := findKind(t, standalone, "ConfigMap")["data"].(map[string]any)["config.yaml"].(string)
	cfg := config.DefaultConfig()
	require.NoError(t, yaml.Unmarshal([]byte(data), cfg))
	require.False(t, cfg.EnabledDistribution)
	require.Equal(t, 1, cfg.MinInSyncReplicas)
	require.Equal(t, 1, cfg.DefaultReplicationFactor)
